...
internal func platformSwiftQualificationQualified(
  ...
  if let optional = type.as(OptionalTypeSyntax.self) {
    guard let inner = platformSwiftQualificationQualified(optional.wrappedType, declared: declared)
    else { return nil }
  ...
  if let attributed = type.as(AttributedTypeSyntax.self) {
    guard let inner = platformSwiftQualificationQualified(attributed.baseType, declared: declared)
    else { return nil }
  ...
  if let someAny = type.as(SomeOrAnyTypeSyntax.self) {
    guard let inner = platformSwiftQualificationQualified(someAny.constraint, declared: declared)
    else { return nil }
