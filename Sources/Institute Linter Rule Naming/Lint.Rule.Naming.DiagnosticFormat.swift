...
internal func namingDiagnosticFormatMatches(_ text: Swift.String) -> Swift.Bool {
    ...
    scalars = scalars.dropFirst()
    // `[\w-]*`
    while let scalar = scalars.first, namingDiagnosticFormatIsWordScalar(scalar) || scalar == "-" {
        scalars = scalars.dropFirst()
    }
    // `\]`
    guard scalars.first == "]" else { return false }
    scalars = scalars.dropFirst()
    // `\s+`
    guard let space = scalars.first, namingDiagnosticFormatIsWhitespaceScalar(space)
    else { return false }
    while let scalar = scalars.first, namingDiagnosticFormatIsWhitespaceScalar(scalar) {
        scalars = scalars.dropFirst()
    }
    ...
    guard let citationFirst = scalars.first, citationFirst != ":" else { return false }
    scalars = scalars.dropFirst()
    ...
    while let scalar = scalars.first, scalar != ":" {
        scalars = scalars.dropFirst()
    }
    guard scalars.first == ":" else { return false }
    scalars = scalars.dropFirst()
    guard let after = scalars.first else { return false }
    return namingDiagnosticFormatIsWhitespaceScalar(after)
}

private func namingDiagnosticFormatIsWordScalar(_ scalar: Swift.Unicode.Scalar) -> Swift.Bool {
    ("a"..."z").contains(scalar) || ("A"..."Z").contains(scalar)
        || ("0"..."9").contains(scalar) || scalar == "_"
}

private func namingDiagnosticFormatIsWhitespaceScalar(
    _ scalar: Swift.Unicode.Scalar
) -> Swift.Bool {
