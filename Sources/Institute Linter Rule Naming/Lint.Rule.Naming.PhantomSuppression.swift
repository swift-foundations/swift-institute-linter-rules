...
fileprivate let namingPhantomSuppressionMessage: Swift.String =
...
internal final class NamingPhantomSuppressionVisitor: SyntaxVisitor {
    ...
    private func checkGenericParameters(_ clause: GenericParameterClauseSyntax?, in decl: Syntax) {
        ...
        for parameter in clause.parameters {
            ...
            guard let inherited = parameter.inheritedType, constraintIsCopyableOnly(inherited) else { continue }
            guard usedAsPhantomDiscriminator(name, in: body), !usedAsStoredValue(name, in: body) else { continue }
