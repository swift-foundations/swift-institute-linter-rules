...
/// explicit display name duplicates the implicit one), so the rule prescribes
/// the fix before the compiler is hit; it is reported with a distinct message.
///
/// Canonical fix: rename the declaration to the backticked descriptive form
/// and drop the string, keeping every trailing trait.
///
...
internal let testingDisplayNameStringMessage: Swift.String =
  ...
  + "check. **Canonical fix**: rename the declaration to the backticked "
  + "descriptive form and drop the string, keeping every trailing trait — "
...
internal let testingDisplayNameDuplicateMessage: Swift.String =
  ...
  + "name repeats the implicit one. **Canonical fix**: delete the string "
  + "argument and keep the backticked declaration name and every trailing "
  + "trait."
...
internal func displayNameCanBeRawIdentifier(_ text: Swift.String) -> Swift.Bool {
  ...
  guard text.contains(where: { !displayNameOperatorCharacters.contains($0) }) else { return false }
  return true
}
...
internal final class TestingDisplayNameStringVisitor: SyntaxVisitor {
  ...
  private func testingAttribute(_ attributes: AttributeListSyntax) -> AttributeSyntax? {
    for attribute in attributes {
      guard let attr = attribute.as(AttributeSyntax.self) else { continue }
      let name = attr.attributeName.trimmedDescription
      for candidate in ["Test", "Suite"] where name == candidate || name.hasSuffix(".\(candidate)")
      {
        return attr
      }
    }
  ...
  private func displayNameLiteral(_ attribute: AttributeSyntax) -> StringLiteralExprSyntax? {
  ...
  private func literalContent(_ literal: StringLiteralExprSyntax) -> Swift.String? {
    var content = ""
  ...
  private func check(name: TokenSyntax, attributes: AttributeListSyntax) {
    guard let attribute = testingAttribute(attributes) else { return }
    guard let literal = displayNameLiteral(attribute) else { return }
    guard let content = literalContent(literal) else { return }
    ...
    let isDuplicate =
      spelled.hasPrefix("`") && spelled.hasSuffix("`")
    ...
    matches.append(
      Diagnostic.Record(
        ...
        message: isDuplicate ? testingDisplayNameDuplicateMessage : testingDisplayNameStringMessage
      ))
  }
