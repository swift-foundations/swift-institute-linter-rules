...
internal final class FrameworkSuiteCategoriesRewriter: SyntaxRewriter {
  ...
  override func visit(_ node: StructDeclSyntax) -> DeclSyntax {
    ...
    guard
      frameworkSuiteCategoriesIsFixEligible(node.memberBlock, missingBareNames: missingBareNames)
    else {
    ...
    let newBlock = node.memberBlock.with(\.members, MemberBlockItemListSyntax(existing + inserted))
